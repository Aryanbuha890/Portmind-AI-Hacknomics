...
def getSingleLineWagonNo(ocrResults, wagonNo):
    ...
    for result in ocrResults:
        ...
        # Display the text if it's somewhat reasonably long (e.g., > 5 chars)
        clean_text = "".join(filter(str.isalnum, text))
        if len(clean_text) >= 5:
            savedWagonNumber = clean_text
            wagonNo = clean_text
...
model_path = os.path.join(current_dir, "models", "wagonNumberDetectionV2.pt")
wadonNumberDetectionModel = YOLO(model_path)
